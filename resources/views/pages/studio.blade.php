...
<div class="editor-container">
    ...
    <div class="main-editor-area">
        ...
        <main class="flex flex-col items-center justify-start p-6 bg-gray-800 relative overflow-auto">
            ...
            <div id="projectTabs" class="flex gap-2 bg-gray-200 px-3 py-2 border-b h-[40px] w-[180px]">
...
<script>
    document.addEventListener("DOMContentLoaded", function () {
        ...
        // tampilkan tab
        addProjectTab(project.id, project.name);

        // aktifkan
        setActiveProject(project.id);
    });

    function addProjectTab(id, name) {
        const tabBar = document.getElementById('projectTabs');

        const tab = document.createElement('div');
        tab.id = id + "-tab";
        tab.className = "px-4 py-1 bg-white border rounded-t cursor-pointer text-black";
        tab.innerText = name;
        //tab.onclick = () => setActiveProject(id);
    ...
    function setActiveProject(id) {
        ...
        // highlight tab
        /*
        document.querySelectorAll("#projectTabs div").forEach(tab => {
            tab.classList.remove("bg-white");
            tab.classList.add("bg-gray-300");
        });

        document.getElementById(id + "-tab").classList.remove("bg-gray-300");
        document.getElementById(id + "-tab").classList.add("bg-white");
        ...
        canvas.innerHTML = `
            <div class="text-center">
                ...
                <div class="text-gray-600 mt-2">Canvas: ${project.width} × ${project.height}</div>
                <div class="mt-4 bg-white shadow-md p-6 rounded">
                    <div style="width:${project.width}px; height:${project.height}px; border:1px solid gray;"></div>
                </div>
            </div>
        `;*/
    }
